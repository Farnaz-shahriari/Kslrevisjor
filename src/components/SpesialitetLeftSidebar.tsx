...
interface LeftSidebarProps {
  ...
  excludedQuestionGroups?: Set<string>;
  filterMode?: 'all' | 'unanswered' | 'documentation';
}
...
export function SpesialitetLeftSidebar({ 
  ...
  excludedQuestionGroups = new Set(),
  filterMode = 'all'
}: LeftSidebarProps) {
  ...
  // Helper function to determine if a question should be shown based on filter mode
  const shouldShowQuestion = (question: any): boolean => {
    ...
    if (filterMode === 'documentation') {
      // Only show questions that require documentation
      return question.requiresDocumentation === true;
    }
  ...
  const getCategoryProgress = (category: SpesialitetMainCategory) => {
  ...
  const isGroupComplete = (group: SpesialitetQuestionGroup) => {
    return group.questions.every(q => q.isAnswered || answeredQuestions.has(q.id));
  };

  // First filter based on manual additions/removals, excluded groups from temaspørsmål, and filter mode, then based on search query
  const baseFilteredCategories = useMemo(() => {
    return mainCategories.map(category => ({
      ...category,
      questionGroups: category.questionGroups
        .filter(group => !excludedQuestionGroups.has(group.id)) // Filter out excluded groups
        .map(group => ({
          ...group,
          questions: group.questions.filter(q => shouldShowQuestion(q))
        }))
  ...
  }, [mainCategories, manuallyAddedQuestions, manuallyRemovedQuestions, excludedQuestionGroups, filterMode, answeredQuestions]);
    ...
    return baseFilteredCategories.map(category => {
      const filteredGroups = category.questionGroups.map(group => {
        ...
        return filteredQuestions.length > 0
          ...
          : null;
      }).filter(Boolean) as SpesialitetQuestionGroup[];

      return filteredGroups.length > 0
        ? { ...category, questionGroups: filteredGroups }
        : null;
    }).filter(Boolean) as SpesialitetMainCategory[];
