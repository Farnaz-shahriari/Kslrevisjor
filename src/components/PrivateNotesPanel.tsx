...
export function PrivateNotesPanel({ isOpen, onClose, onUpdateQuestionData, getQuestionData, savedNotes = [], onSaveNote, onDeleteNote }: PrivateNotesPanelProps) {
  ...
  const handleNoteClick = (note: SavedNote) => {
    ...
    if (note.isDeviation) {
      setDeviationData({
        severity: note.deviationSeverity,
        mangel: note.deviationMangel,
        bevis: note.deviationBevis,
        krav: note.deviationKrav,
      });
    }
  ...
  // Render notes list view in bottom sheet
  ...
  // Render note creation/editing view in bottom sheet
  function renderNotesPanelContent() {
    // Show inline voice recording UI when recording
    if (isRecording) {
      return (
        <BottomSheet isOpen={isOpen} onClose={handleClose} snapPoints={[0.9]}>
          <InlineVoiceRecording onConfirm={handleConfirmRecording} onCancel={handleCancelRecording} />
        </BottomSheet>
      );
    }
  ...
  return viewMode === 'list' ? renderNotesListView() : renderNotesPanelContent();
}