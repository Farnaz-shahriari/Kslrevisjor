...
export function TildelteRevisjonerPage({ onRevisionClick }: TildelteRevisjonerPageProps) {
  ...
  const tildelteRevisjoner: TildeltRevisjon[] = [
    {
      ...
      revisjonData: {
        ...
        isPriority: true,
        isOppfolgingGyldigKSL: true,
      },
      location: { lat: 58.1556, lng: 8.2088 },
    },
    {
      id: '2',
      foretakName: 'Nordlys Økologiske',
      visitDate: new Date(2025, 1, 8),
      ...
      location: { lat: 58.0876, lng: 7.8011 },
    },
    {
      id: '3',
      foretakName: 'Grønn Vallei Gård',
      visitDate: new Date(2025, 1, 12),
      ...
      location: { lat: 58.2767, lng: 7.9572 },
    },
    {
      id: '4',
      foretakName: 'Fjellheim Sauebruk',
      visitDate: new Date(2025, 1, 15),
      ...
      location: { lat: 58.2011, lng: 7.7453 },
    },
    {
      id: '5',
      foretakName: 'Solsiden Frukt & Bær',
      visitDate: new Date(2025, 1, 18),
      ...
      location: { lat: 58.2475, lng: 8.3788 },
    },
    {
      id: '6',
      foretakName: 'Aurland Biogård',
      visitDate: new Date(2025, 1, 20),
      ...
      location: { lat: 58.3522, lng: 8.2578 },
    },
    {
      id: '7',
      foretakName: 'Eikestad Melkeproduksjon',
      visitDate: new Date(2025, 1, 22),
      ...
      location: { lat: 58.4367, lng: 7.9583 },
    },
    {
      id: '8',
      foretakName: 'Høgås Gårdsdrift',
      visitDate: new Date(2025, 2, 3),
      visitTime: '09:00 - 11:30',
      revisjonData: {
        ordning: 'Spesialitet',
      ...
      location: { lat: 58.5889, lng: 7.7967 },
    },
    {
      id: '9',
      foretakName: 'Lunden Grønnsaker',
      visitDate: new Date(2025, 2, 7),
      visitTime: '13:30 - 16:00',
      revisjonData: {
        ordning: 'LokalMat',
        revisjonsfrist: '14. mars 2025',
        produksjon: ['Økologiske grønnsaker', 'Rotfrukter', 'Urter'],
        kommune: 'Kristiansand',
        address: 'Justnesveien 74, 4626 Kristiansand',
        isPriority: true,
      },
      location: { lat: 58.1889, lng: 8.0444 },
    },
    {
      id: '10',
      foretakName: 'Solberg Økologisk',
      visitDate: new Date(2025, 2, 10),
      visitTime: '15:00 - 17:00',
      revisjonData: {
        ordning: 'Nyt Norge',
        revisjonsfrist: '17. mars 2025',
        produksjon: ['Geit', 'Geitost produksjon', 'Grovfôr'],
        kommune: 'Lindesnes',
        address: 'Åros 33, 4521 Lindesnes',
        isPriority: false,
      },
      location: { lat: 58.0367, lng: 7.0500 },
    },
  ];
  ...
  const getFilteredRevisjoner = useMemo(() => {
    let filtered = [...tildelteRevisjoner];
    ...
    if (selectedRevisjonstyper.length > 0) {
      filtered = filtered.filter(rev => {
        ...
        if (selectedRevisjonstyper.includes('oppfolging-gyldig-ksl') && rev.revisjonData.isOppfolgingGyldigKSL) return true;
        if (selectedRevisjonstyper.includes('ordinar') && !rev.revisjonData.isPriority && !rev.revisjonData.isOppfolgingGyldigKSL) return true;
        return false;
      });
    }
    ...
    // Filter by Akseptfrist (visitDate)
    if (akseptfristFrom) {
      filtered = filtered.filter(rev => rev.visitDate >= akseptfristFrom);
    }
    if (akseptfristTo) {
      filtered = filtered.filter(rev => rev.visitDate <= akseptfristTo);
    }

    return filtered;
  }, [tildelteRevisjoner, selectedRevisjonstyper, selectedProduksjon, selectedOrdning, selectedKommune, akseptfristFrom, akseptfristTo]);

  // Extract unique values with counts from the original data (not filtered)
  const filterOptions = useMemo(() => {
    ...
    const revisjonstypeCounts = {
      ...
      'oppfolging-gyldig-ksl': 0,
      'ordinar': 0,
    };

    tildelteRevisjoner.forEach(rev => {
      // Count ordning
      ...
      // Count revisjonstype
      if (rev.revisjonData.isPriority) revisjonstypeCounts['prioritert-revisjon']++;
      if (rev.revisjonData.isOppfolgingGyldigKSL) revisjonstypeCounts['oppfolging-gyldig-ksl']++;
      if (!rev.revisjonData.isPriority && !rev.revisjonData.isOppfolgingGyldigKSL) revisjonstypeCounts['ordinar']++;
    });
    ...
    return {
      ...
      revisjonstype: [
        ...
        { value: 'oppfolging-gyldig-ksl', label: 'Oppfølging gyldig KSL', count: revisjonstypeCounts['oppfolging-gyldig-ksl'] },
        { value: 'ordinar', label: 'Ordinær', count: revisjonstypeCounts['ordinar'] },
      ].filter(item => item.count > 0), // Only show if count > 0
    };
  }, [tildelteRevisjoner]);
  ...
  const getSortedRevisjoner = () => {
    ...
    if (sortBy === 'akseptfrist') {
      ...
      sorted.sort((a, b) => a.revisjonData.revisjonsfrist.localeCompare(b.revisjonData.revisjonsfrist));
  ...
  const getSortLabel = () => {
    switch (sortBy) {
      case 'akseptfrist': return 'Akseptfrist';
  ...
  const revisjonstypeLabels: Record<string, string> = {
    ...
    'oppfolging-gyldig-ksl': 'Oppfølging gyldig KSL',
    'ordinar': 'Ordinær',
  };
